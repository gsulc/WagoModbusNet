...
namespace WagoModbusNet
{
    public abstract class ModbusMaster
    {
        ...
        /// </summary>
        /// <param name="id">Unit-Id or Slave-Id</param>
        ...
        public bool[] ReadCoils(byte id, ushort startAddress, ushort readCount)
        {
            ...
            {
                data[i] = (responsePdu[k + 3] & (byte)(0x01 << (i % 8))) > 0;
                k = (i + 1) / 8;
            }
        ...
        {
            byte[] responsePdu = SendModbusRequest(
                id, ModbusFunctionCodes.ReadHoldingRegisters, startAddress, readCount, 0, 0, null);
        ...
        public void MaskWriteRegister(byte id, ushort startAddress, ushort andMask, ushort orMask)
        {
            ...
            byte[] tmp = BitConverter.GetBytes(andMask);
            writeData[0] = tmp[0];
            writeData[1] = tmp[1];
        ...
        public ushort[] ReadWriteMultipleRegisters(byte id, ushort readAddress, ushort readCount, ushort writeAddress, ushort[] writeData)
        {
            ...
            {
                byte[] tmp = BitConverter.GetBytes(writeData[i]);
                writeBuffer[k] = tmp[1];
        ...
        internal bool IsModbusException(byte b) => (b & 0x80) > 0;
